...
const NavDropdownButton = ({name,cat,subcat}) => {
    ...
    const category = categories.map((category) =>
        <li key={category._id}>
            <Link className=" bg-gray-200 hover:bg-gray-400 py-2 px-4 block whitespace-no-wrap" to={`${category.slug}`}>
                { category.name }
            </Link>
        </li>
    );
    ...
    const subCategory = subCategories.map((subCategory) =>
        <li key={subCategory._id}>
            <Link className=" bg-gray-200 hover:bg-gray-400 py-2 px-4 block whitespace-no-wrap" to={`${subCategory.slug}`}>
                { subCategory.name }
            </Link>
        </li>
    );
    return (
        <div className="border-black border-[1px] flex bg-gray-700 mx-2 my-2 rounded-lg text-center justify-center items-center p-4 hover:scale-[1.01] duration-500 transition hover:bg-main-blue/40 hover:border-white z-20">

            <div className="inline-block relative group ">
                <button className="font-semibold text-white rounded inline-flex items-center">
